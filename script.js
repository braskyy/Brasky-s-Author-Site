...
// Note: Each object here represents a "Step". 
// The voice reads the text, waits for the voice to finish, then waits for the 'delay' (ms) before clicking to the next step.
// 'split' steps use the same ID to simulate multiple clicks on one slide.

const shapesScript = [
    // SLIDE 1: Intro (Split into 4 "Clicks" for animations)
    ...
    // SLIDE 2: History (Split into chunks to pace animations if needed, or just for reading clarity)
    ...
    { id: "g70c6d348ce_0_37", text: "My body is the shape I live in and it shapes the way I live. Raquel Welch said this while she was staring at the clouds trying to determine what shapes they looked like to her. She did such a good job at this that she was awarded an honorary degree at Oxford University. It was actually called Boxford University at the time, but she convinced the chancellor to change the name to Oxford by reasoning the first letter in the word Oxford, ‘O,’ could actually be considered a circle, one of mankind’s earliest known shapes. And you’ll no doubt recognize the connection between this Oxford Circle and the celebrated Harvard Square in America. Suncube, square dance, circle pit? Let’s find out! THE END.", delay: 5000 }
];

let currentSlideIdx = 0;
let lastSrc = ""; 
// Added delayms=60000 to prevent Google's auto-advance from conflicting with our script
...
function startShapesEgg() {
    ...
    // UPDATED: Volume increased to 30%
    audio.volume = 0.3;
...
function speakSlide(idx) {
    ...
    // 1. Update Slide only if the ID has changed 
    ...
    // 3. Automated Voice Setup
    const utterance = new SpeechSynthesisUtterance(shapesScript[idx].text);
    
    let voices = window.speechSynthesis.getVoices();
    // Prioritize Female voices more aggressively
    const preferredVoice = voices.find(v => v.name.includes('Google US English') || v.name.includes('Female'));
    if (preferredVoice) utterance.voice = preferredVoice;

    // UPDATED: Voice Settings for "Old Woman" feel
    // Pitch 1.1 (slightly higher/frailer than deep male) 
    // Rate 0.85 (slow and deliberate)
    utterance.pitch = 1.1; 
    utterance.rate = 0.85; 

    // 4. Sync Ticker Speed
    // Calculate animation duration based on word count and speech rate
    // Logic: (WordCount / RateFactor) + buffer
    const wordCount = shapesScript[idx].text.split(' ').length;
    // This formula approximates the time it takes for the ticker to scroll fully at reading speed
    const estDuration = Math.max(5, (wordCount * 0.7)); 
    tickerWrap.style.animationDuration = `${estDuration}s`;
    
    // Reset animation to ensure it starts from beginning for new text
    ...
    utterance.onend = () => {
        ...
        if (currentSlideIdx < shapesScript.length) {
            // UPDATED: Use the specific delay from the script object
