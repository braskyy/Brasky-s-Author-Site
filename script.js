...
const shapesScript = [
    { text: "The History of Shapes--A TedX Talk by Dr. Triangela Shapiro, Shape Hero" },
    ...
    { text: "Likewise, the rhomboids, the trapezius, and even the pronator quadratus all ride silently under our skin as we exchange cloth rectangles and copper discs for circle sectors with extra cheese and piping hot columns of coffee." },
...
function startShapesEgg() {
    ...
    // Set to Slide 1 (Title)
    currentSlideIndex = 0;
    currentTextIndex = 0;
    updateSlide(0);

    // Initial 4 seconds of silence while Slide 1 is visible
    setTimeout(() => {
        speakStep(0);
    }, 4000);
}
...
function updateSlide(index) {
    const iframe = document.getElementById('shapes-frame');
    // Using index-based navigation is more reliable for Google Slides
    iframe.src = `https://docs.google.com/presentation/d/${slideId}/embed?start=false&loop=false&rm=minimal&slide=index.${index}`;
}

function speakStep(textIdx) {
    if (textIdx >= shapesScript.length || document.getElementById('shapes-overlay').style.display === 'none') return;

    const synth = window.speechSynthesis;
    const utterance = new SpeechSynthesisUtterance(shapesScript[textIdx].text);
    ...
    utterance.onend = () => {
        // Logic for advancing based on your "clicks" map
        let jump = 1;
        if (textIdx === 0) jump = 4; // S1 -> S2 requires 4 clicks
        else if (textIdx === 4) jump = 3; // S5 -> S6
        else if (textIdx === 5) jump = 3; // S6 -> S7
        else if (textIdx === 6) jump = 3; // S7 -> S8
        else if (textIdx === 7) jump = 2; // S8 -> S9
        else if (textIdx === 8) jump = 2; // S9 -> S10
        else if (textIdx === 9) jump = 3; // S10 -> S11
        // ... (remaining slides follow similarly)

        currentSlideIndex += jump;
        
        // Wait 1.5 seconds before showing next slide and speaking
        setTimeout(() => {
